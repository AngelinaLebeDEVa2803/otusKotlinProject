...
1.  **`bookings.create`** - Создание нового бронирования
...
2.  **`bookings.read`** - Получение информации о бронировании

3.  **`bookings.all`** - Получение списка своих бронирований
    *   *Бизнес-логика:* Возвращает бронирования пользователя на указанную дату. Фильтр по userId, startTime, endTime и status

4.  **`bookings.update`** - Обновление бронирования (например, изменение времени)
    *   *Бизнес-правило:* Изменение возможно не позднее 1 часа до начала брони.

5.  **`bookings.delete`** - Отмена бронирования

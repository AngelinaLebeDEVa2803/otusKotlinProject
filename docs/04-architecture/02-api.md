...
Здания:
1.  **`POST /api/v1/buildings`** - Внесение нового филиала в базу
...
2.  **`GET /api/v1/buildings/{buildingId}`** - Получение информации о здании

3.  **`GET /api/v1/buildings`** - Получение списка всех зданий в заданном филиале
    *   *Параметры:* `?filialId=001`

4.  **`PATCH /api/v1/buildings/{buildingId}`** - Обновление информации о здании

5.  **`DELETE /api/v1/buildings/{buildingId}`** - Удаление здания из базы
...
2.  **`GET /api/v1/rooms/{roomId}`** - Получение информации о помещении
...
4.  **`PATCH /api/v1/rooms/{roomId}`** - Обновление информации о помещении

5.  **`DELETE /api/v1/rooms/{roomId}`** - Удаление помещения из базы
...
2.  **`GET /api/v1/bookings/{bookingId}`** - Получение информации о бронировании
...
4.  **`PATCH /api/v1/bookings/{bookingId}`** - Обновление бронирования (например, изменение времени)
    *   *Бизнес-правило:* Изменение возможно не позднее 1 часа до начала брони.

5.  **`DELETE /api/v1/bookings/{bookingId}`** - Отмена бронирования
    *   *Бизнес-правило:* Отмена возможна в любое время до начала брони.

**Поиск и доступность**

6.  **`GET /api/v1/rooms/availability`** - Поиск свободных переговорок и отображение броней
...
6.  **`GET /api/v1/workspaces/availability`** - Поиск свободных мест и отображение броней
...
7.  **`GET /api/v1/rooms/{roomId}/availability`** - Проверка доступности конкретного помещения (для переговорок)
...
7.  **`GET /api/v1/workspaces/{workspaceId}/availability`** - Проверка доступности конкретного рабочего места
    *   *Возвращает:* `{ "available": true }` или `{ "available": false, "userId": "1234", "nextAvailable": "2024-09-20T18:00:00Z" }`

**Просмотр команды**
6.  **`GET /api/v1/team`** - Список бронирований членов команды за определенный период (для тимлида)
    *   *Бизнес-логика:* Возвращает список бронирований членов команды за определенный период
    *   *Параметры:* `?startTime=2024-09-20T09:00:00Z&endTime=2024-09-20T18:00:00Z`

**Аналитика**

8. **`GET /api/v1/analytics/usage`** - Получение аналитики использования (для администраторов)
...
1.  **Скорость ответа API:** 95% запросов к `/api/v1/availability` должны обрабатываться ≤ 500 мс.

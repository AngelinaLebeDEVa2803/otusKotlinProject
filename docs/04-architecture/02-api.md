...
7.  **`GET /api/v1/workspaces/{workspaceId}/availability`** - Проверка доступности конкретного места
    *   *Возвращает:* `{ "available": true }` или `{ "available": false, "nextAvailable": "2024-09-20T18:00:00Z" }`

**Управление картой и администрирование**
8.  **`GET /api/v1/floors/{floorId}/map`** - Получение карты этажа
    *   *Возвращает:* SVG или JSON с данными о расположении всех мест на этаже и их текущим статусом.


10. **`GET /api/v1/analytics/usage`** - Получение аналитики использования (для администраторов)

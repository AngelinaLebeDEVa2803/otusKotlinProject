...
fun ICorChainDsl<OfficeContext>.stubValidationBadRoomId(title: String) = worker {
    this.title = title
    this.description = """
        Кейс ошибки валидации для идентификатора roomId
    """.trimIndent()
    on { stubCase == OfficeStubs.BAD_ROOM_ID && state == OfficeState.RUNNING }
    handle {
        fail(
            OfficeError(
                group = "validation",
                code = "validation-roomId",
                field = "roomId",
                message = "Wrong roomId field"
            )
        )
    }
}

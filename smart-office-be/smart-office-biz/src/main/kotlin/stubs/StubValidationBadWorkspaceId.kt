...
fun ICorChainDsl<OfficeContext>.stubValidationBadWorkspaceId(title: String) = worker {
    this.title = title
    this.description = """
        Кейс ошибки валидации для идентификатора workspaceId
    """.trimIndent()
    on { stubCase == OfficeStubs.BAD_WORKSPACE_ID && state == OfficeState.RUNNING }
    handle {
        fail(
            OfficeError(
                group = "validation",
                code = "validation-workspaceId",
                field = "workspaceId",
                message = "Wrong workspaceId field"
            )
        )
    }
}

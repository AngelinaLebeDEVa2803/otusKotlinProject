...
fun ICorChainDsl<OfficeContext>.stubCannotDelete(title: String) = worker {
    this.title = title
    this.description = """
        Невозможно удалить бронирование (например, уже начато)
    """.trimIndent()
    on { stubCase == OfficeStubs.CANNOT_DELETE && state == OfficeState.RUNNING }
    handle {
        fail(
            OfficeError(
                group = "logic",
                code = "logic-cannotDelete",

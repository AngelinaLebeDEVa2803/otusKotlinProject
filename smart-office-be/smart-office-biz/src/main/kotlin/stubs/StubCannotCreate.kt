...
fun ICorChainDsl<OfficeContext>.stubCannotCreate(title: String) = worker {
    this.title = title
    this.description = """
        Невозможно создать бронирование (место занято, лимит броней превышен etc)
    """.trimIndent()
    on { stubCase == OfficeStubs.CANNOT_CREATE && state == OfficeState.RUNNING }
    handle {
        fail(
            OfficeError(
                group = "logic",
                code = "logic-cannotCreate",

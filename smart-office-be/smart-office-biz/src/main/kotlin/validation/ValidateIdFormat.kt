...
fun ICorChainDsl<OfficeContext>.validateIdFormat(title: String) = worker {
    this.title = title

    // пусть в формате uuid
    val regExp = Regex("^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$")
    on { bookingValidating.id != OfficeBookingId.NONE && !bookingValidating.id.asString().matches(regExp) }
    handle {

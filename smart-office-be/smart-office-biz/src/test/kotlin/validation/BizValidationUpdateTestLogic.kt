...
class BizValidationUpdateTestLogic: BaseBizValidationTest() {
    override val command = OfficeCommand.UPDATE

    @Test
    fun correctStatusFuture() = runTest {
    ...
    fun correctStatusPast() = runTest {
        val ctx = OfficeContext(
            ...
            bookingRequest = OfficeBooking(
                ...
                status = OfficeBookingStatus.COMPLETED,
                lock = OfficeBookingLock("123"),
            )
        )
        processor.exec(ctx)
    ...
    @Test
    fun incorrectStatusNone() = runTest {
        val ctx = OfficeContext(
            command = command,
            state = OfficeState.NONE,
            workMode = OfficeWorkMode.TEST,
    ...
    @Test
    fun incorrectStatusFuture() = runTest {
        val ctx = OfficeContext(
            command = command,
            state = OfficeState.NONE,
            workMode = OfficeWorkMode.TEST,
    ...
    @Test
    fun incorrectStatusPast() = runTest {
        val ctx = OfficeContext(
            command = command,
            state = OfficeState.NONE,
            workMode = OfficeWorkMode.TEST,
            bookingRequest = OfficeBooking(
                ...
                roomId = OfficeRoomId("room_404"),
                workspaceId = OfficeWorkspaceId("009"),
                startTime = Instant.parse("2020-03-30T09:00:00Z"),
                endTime = Instant.parse("2020-03-30T18:00:00Z"),
